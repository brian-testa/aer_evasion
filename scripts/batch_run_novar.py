...
evasive_todo_tmp = pickle.load(open(os.path.join(jar, 'evasive_todo.pickle'), 'rb'))
evasive_todo = {}
for key in evasive_todo_tmp.keys():
    if not audiodatasets.is_variant(key):
        evasive_todo[key] = evasive_todo_tmp[key]
########################################################
#
